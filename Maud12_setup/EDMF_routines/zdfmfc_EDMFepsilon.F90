...
CONTAINS
   ...
   SUBROUTINE tra_mfc( kt, Kmm, pts, Krhs )
      ...
      !
      REAL(wp), DIMENSION(A2D(nn_hls)) :: zepsT  !
     ...
     !
      ...
      zfbuo(:,:)   = 0._wp
      ! BG: replace fixed values for alpha and beta with the ones from the
      ! memory
      WHERE ( ABS(zrautb(:,:)) > 1.e-20 ) zfbuo(:,:)   =   &
         &      grav * ( rab_n(:,:,1,jp_tem) *r1_rho0 *zfnet(:,:)              &
         &      - rab_n(:,:,1,jp_sal)*r1_rho0 *pts(A2D(nn_hls),1,jp_sal,Kmm)  &
         &      * zsws(:,:)/zrautb(:,:)) * zhcmo(:,:)
      ! Original code for EDMF
      !WHERE ( ABS(zrautb(:,:)) > 1.e-20 ) zfbuo(:,:)   =   &
      !   &      grav * ( 2.e-4_wp *zfnet(:,:)              &
      ...
      DO jk= 2, jpk
         ...
         zrautbm1(:,:) = zrautb(:,:)
         ! BG: change density within the ocean from potential density to insitu
         ! density - use eos_insitu_2d(pts,pdep,prd)
         CALL eos( pts (:,:,jk  ,:,Kmm), gdept(:,:,jk  ,Kmm) , zrautb(:,:)  )
         CALL eos( ztsp(:,:,jk-1,:    ), gdept(:,:,jk-1,Kmm) , zraupl(:,:)  )
         ! Original code for EDMF - uses the subroutine eos_insitu_pot_2d 
         !CALL eos( pts (:,:,jk  ,:,Kmm) ,  zrautb(:,:)   )
         !CALL eos( ztsp(:,:,jk-1,:    ) ,  zraupl(:,:)   )

         ! BG: pressure not needed anymore
         !DO_2D( 0, 0, 0, 0 )
         ...
         ! BG: rho gives density anomaly, not density!!
         ! WHERE(zrautbm1 .NE. 0.) zfbuo(:,:)  =  grav * (zraupl(:,:) - zrautbm1(:,:)) / zrautbm1(:,:)
         WHERE(zrautbm1 .NE. 0.) zfbuo(:,:)  =  grav * (zraupl(:,:) - zrautbm1(:,:)) !/ (1 + zrautbm1(:,:))
         ...
         DO_2D( 0, 0, 0, 0 )
            ...
            zcnh = 0.2_wp 
            ! BG: can be simplified noting that g*rho/p = 1/z. use z+10m to
            ! avoid division by zero
            znum = 0.5_wp  + zcnh - (zcnh/(gdept(ji,jj,jk,Kmm)+10._wp)+zcb*zepsT(ji,jj)) &
                   *e3t(ji,jj,jk-1,Kmm)*0.5_wp
            zden = 0.5_wp  + zcnh + (zcnh/(gdept(ji,jj,jk,Kmm)+10._wp)+zcb*zepsT(ji,jj)) &
                   *e3t(ji,jj,jk-1,Kmm)*0.5_wp
            ! Original Code
            !znum = 0.5_wp  + zcnh - &
            ...
            !----------------------------------------------------------------------------------
            ! BG: avoid negative case for constant value for speed
            ! IF( rn_cap .GT. 0. ) THEN
               ...
               IF( zxw > 0. ) THEN
                  ...
                  ! BG: whatever the case, zctre - zcdet = -rn_cap*zxl
               ...
               ELSE
                   ! BG: introduce a nonlinear dependence in wp sign 
            ...
            ! BG: very vectorized. Use non-constant case only
            ! Background value is 0.001/m, not 0.01/m !?
            ! IF(rn_cemf .GT. 0.) THEN
               zxw = 0.5_wp*(zrwp(ji,jj,jk-1)+ zrwp(ji,jj,jk) )
               zepsT(ji,jj)  =  0.001_wp
               IF( zxw > 0.  ) THEN
                  zepsT(ji,jj)  =  zepsT(ji,jj) +                       &
                                   &  ABS( zrwp(ji,jj,jk-1)-zrwp(ji,jj,jk) )  &
                                   &  / ( e3t(ji,jj,jk-1,Kmm) * zxw )         &
                                   & * rn_cemf * wmask(ji,jj,jk)
               ENDIF
            !ELSE
            !   zepsT(ji,jj,jk)  = -rn_cemf
            !ENDIF

            ! BG: zepsT used instead of zepsW
            ...
            zcoef1 =  (1._wp-zepsT(ji,jj)*(1._wp-zrw)*e3w(ji,jj,jk,Kmm)*wmask(ji,jj,jk ) ) &
            &       / (1._wp+zepsT(ji,jj)*zrw*e3w(ji,jj,jk,Kmm)*wmask(ji,jj,jk) )
            !
            zcoef2 =  zepsT(ji,jj)*e3w(ji,jj,jk,Kmm)*wmask(ji,jj,jk)                       &
            &       / (1._wp+zepsT(ji,jj)*zrw*e3w(ji,jj,jk,Kmm)*wmask(ji,jj,jk))
            !
